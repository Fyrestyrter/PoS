    ...
    if request.POST:
        ...
        if user is not None:
            if user.is_active:
                ...
                resp['msg'] = "Неверный логин или пароль"
        else:
            resp['msg'] = "Неверный логин или пароль"
...
def save_product(request):
    ...
    if len(check) > 0 :
        resp['msg'] = "Такой код продукта уже существует"
    else:
...
def save_pos(request):
    ...
    try:
        ...
        messages.success(request, "Запись продажи была сохранена.")
    except:
...
def delete_sale(request):
    ...
    try:
        ...
        resp['status'] = 'success'
        messages.success(request, 'Запись продажи была удалена.')
    except:
        resp['msg'] = "An error occured"
        print("Unexpected error:", sys.exc_info()[0])
    return HttpResponse(json.dumps(resp), content_type='application/json')